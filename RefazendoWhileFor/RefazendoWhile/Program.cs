...
namespace RefazendoWhile
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var wordCount = FindyByWhile(Console.ReadLine());

            Console.WriteLine($"Encontados:{wordCount}");
            Console.ReadKey();
        }
        ...
        /// </summary>
        /// <param name="wordFind">Parametro de 3 caracteres</param>
        /// <returns>A quantidade encontrada da expressão</returns>

        private static int FindyByWhile(string wordFind)
        {
            var contentToFind = TextContent();
            var wordFindCountLenght = wordFind.Length;
            var lenghtText = contentToFind.Length - (wordFindCountLenght - 1);
            var Berenilson = 0;
            var Peraltson = 0;

            while (Peraltson < (lenghtText))
...

            {
                ...
                {
                    compareWord += contentToFind[Peraltson + b].ToString();
                    b++;
                }
        ...
        /// </summary>
        /// <returns> conteúdo do texto em string</returns>
